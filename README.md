...
| ファイル | 内容 |
| --- | --- |
| `index.html` | トップページ。「FVPとWPWの鑑別資料」の行までは従来のまま、その下を `assets/tsugu.css` で作り直し |
| `index_2608version.html` | 作り直す前のトップページ（2026/9/21版） |
| `index_codex.html` | Codex が作った案（参考） |
| `media.html` | 新聞記事・テレビ報道 |
| `society.html` | 学会での仕事と、2016年以降の講演・セミナー記録 |

### 新聞記事の超拡大スクロール（`media.html`）

記事ごとの `<section class="zs">` に次の2つを書くと、スクロールに合わせて
「全体 → 注目点を文字が読める大きさ（画面上で1文字約17〜21px）まで拡大 → 次の注目点 → 全体」と動く。

- `data-char` : 元画像での本文1文字の大きさ（px）。倍率の基準
- `data-path` : 読む順に並べた注目点の JSON。`x`,`y` は画像内の位置（0〜1）、`z` は拡大度（0=全体、1=本文が読める大きさ）、`w` は横書きの行幅（行が画面からはみ出さないよう倍率を抑える）、`t` は表示する説明
